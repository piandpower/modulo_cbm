...
//	require_once('PHP/funciones.php');
    require_once('sentenciasBorrar.php');

   // if (empty($_GET["id"])) { $id=0;} 
  //  else { $id = $_GET["id"];}

 //	global $id;
		$db = pg_pconnect("host=172.16.1.179 port=5432 dbname=metadatos user=gmagallanes password=gmagallanes-2016") or die( "Error al conectar a Metadatos: ".pg_last_error() );
           	if (!$db) { exit('Error en la conexión a la base de datos METADATOS'); } 
  // 	else{ return $db;} 


//    	$cat = pg_pconnect("host=172.16.1.179 port=5432 dbname=catalogos user=gmagallanes password=gmagallanes-2016") or die( "Error al conectar a Catalogos: ".pg_last_error() );
...
//	$db = conectar();
	...
	if ($db)
	{
		...
		if (!$res_taxon) { exit("Error de consulta. Y el id es: ".$id."Aqui"); }
